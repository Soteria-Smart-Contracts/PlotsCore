...
contract PlotsCore {
    ...
    function CloseLoan(address Collection, uint256 ID) public{
        require(AllLoans[AllLoansIndex[Collection][ID]].Borrower == msg.sender || Admins[msg.sender] || AllLoans[AllLoansIndex[Collection][ID]].Lender && UsageExpirationUnix[], "Not owner of loan");
            "Invalid loan"
        );
