...
contract PlotsCoreV1 {
    ...
    function BorrowToken(address Collection, uint256 TokenId, LengthOption Duration, OwnershipPercent Ownership) public payable {
        ...
        if(ListingsByCollection[Collection][TokenIndex].OwnershipOption == ListingType.Ownership){
            ...
            require(Ownership == OwnershipPercent.Zero && PlotsLendV1(LendContract).GetTokenLocation(Collection, TokenId) == LendContract, "Invalid ownership or token location");
    ...
    function CloseLoan(address LoanContract, bool relist) public{
        require(
            ...
            (NFTLoan(LoanContract).LoanEndTime() <= block.timestamp || Admins[msg.sender]) &&
        ...
        if(relist == true){
            ...
            AddListingToUser(msg.sender, Collection, TokenId, Listing(Lender, Collection, TokenId, ListingType.Usage));
        }
    }
...
contract PlotsTreasuryV1{
    ...
    function SellVLND(uint256 Amount, uint256 minOut) public {
        ...
        require(((address(this).balance - PlotsCoreV1(PlotsCoreContract).LockedValue()) - Value) >= ((GetTotalValue() * 95) / 100), "Not enough ether in treasury, must leave 5%");
